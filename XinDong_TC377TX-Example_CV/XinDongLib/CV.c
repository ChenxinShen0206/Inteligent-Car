...
void CV_PreprocessImage(uint16 *input_img, 
                       uint16 *mask) {
    uint16 y, x;
    const uint16 threshold = 90; // 手动阈值，可按需调整
...
CV_Result_t CV_DetectMidline(uint16 *input_img) {
    ...
    uint16 row[5] = {100, 110, 120, 130, 140};
    uint16 mid_x[5];
    float slope;
    ...
    result.error = CV_CalculateMidlineError(input_img, temp);
    result.valid = 1;
    result.angle = CV_CalcMidlineSlopeAngle(&temp, row, 5, mid_x, &slope);
