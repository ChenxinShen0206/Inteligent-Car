/**********************************************************************************************************************
 * Author: Duffy
 * Description:
 * 
 * Version:
 *********************************************************************************************************************/

#ifndef XINDONGLIB_CV_H_
#define XINDONGLIB_CV_H_

/*********************************************************************************************************************/
/*-----------------------------------------------------Includes------------------------------------------------------*/
/*********************************************************************************************************************/
#include "Camera.h"
#include "XinDong_Config.h"

/*********************************************************************************************************************/
/*------------------------------------------------------Macros-------------------------------------------------------*/
/*********************************************************************************************************************/
#define CV_IMAGE_WIDTH      CAM_IMAGE_WIDTH
#define CV_IMAGE_HEIGHT     CAM_IMAGE_HEIGHT
#define CV_TARGET_Y         CAM_IMAGE_HEIGHT * 1 / 2               /*填入合适的值   2pt */  // 指定提取中点的纵轴位置
#define CV_SCAN_START_Y     CV_TARGET_Y - 10  // 补线判断的起始Y坐标
#define CV_SCAN_END_Y       CV_TARGET_Y + 10  // 补线判断的结束Y坐标
#define CV_RI_TARGET_Y      CAM_IMAGE_HEIGHT * 2 / 3
/*********************************************************************************************************************/
/*-------------------------------------------------Global variables--------------------------------------------------*/
/*********************************************************************************************************************/
extern sint16 g_buxian;  // 补线方向判断变量

/*********************************************************************************************************************/
/*-------------------------------------------------Data Structures---------------------------------------------------*/
/*********************************************************************************************************************/
typedef struct {
    uint16* image_data;
    sint16 error;
    uint16 valid;
    uint16 left;
    uint16 right;
} CV_Result_t;

/*********************************************************************************************************************/
/*--------------------------------------------Private Variables/Constants--------------------------------------------*/
/*********************************************************************************************************************/

/*********************************************************************************************************************/
/*------------------------------------------------Function Prototypes------------------------------------------------*/
/*********************************************************************************************************************/
// 图像预处理：转换为二值化掩码（输入/掩码一维展开）
void CV_PreprocessImage(uint16 *input_img,
                       uint8 *mask) ;
// 检查指定区域是否全为黑色（掩码一维展开）
uint16 CV_IsRegionEmpty(uint16 *mask,
                      uint16 y, uint16 start_x, uint16 end_x);

// 计算指定区域的平均位置（掩码一维展开）
uint16 CV_CalculateAveragePosition(uint16 *mask,
                                  uint16 y, uint16 start_x, uint16 end_x);

// 中线检测主函数（输入一维展开）
CV_Result_t CV_DetectMidline(uint16 *input_img, uint8* flag);

// 计算图像中线的偏差（输入/掩码一维展开）
sint16 CV_CalculateMidlineError(uint16 *input_img,
                              uint16 *mask);

// 图像处理主函数（返回 image_data 为一维展开首地址）
CV_Result_t CV_ProcessImage(uint8* flag);

// 获取中线左右两侧的颜色值（输入一维展开）
void CV_GetMidlineSideColors(uint16 *input_img,
                            uint16 y_position, uint16 left_range, uint16 right_range,
                            uint16* left_color, uint16* right_color);

// 获取动态中线左右两侧的颜色值（基于检测到的中线位置，输入/掩码一维展开）
void CV_GetDynamicMidlineSideColors(uint16 *input_img,
                                   uint16 *mask,
                                   uint16 y_position, uint16 left_range, uint16 right_range,
                                   uint16* left_color, uint16* right_color, uint16* midline_x);

// 计算图像区域内中线左侧白色像素点数量和右侧像素点数量差值
sint32 CV_CalculateLeftRightPixelError(uint16 *mask, 
                                    uint16 scan_start_y, 
                                    uint32 *left_count, 
                                    uint32 *right_count);
//环岛检测
uint8 CV_RI_Detect(uint8* Output_Mask);
#endif /* XINDONGLIB_CV_H_ */
