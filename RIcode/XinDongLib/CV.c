...
void CV_PreprocessImage(uint16 *input_img, 
                       uint8 *mask) {
    uint16 y, x;
    const uint8 threshold = 65; // 手动阈值，可按需调整
    ...
    uint8 temp[CV_IMAGE_WIDTH * CV_IMAGE_HEIGHT];
...
//环岛检测
uint8 CV_RI_Detect(uint8* Output_Mask){
    sint16 y;
    sint16 flag = 0;
    uint16 left_Black,left;
    uint16 half = CV_IMAGE_WIDTH / 2;

    //识别是否存在锐角
    for(y = CV_RI_TARGET_Y - 3;y <= CV_RI_TARGET_Y + 3;y++){
        left = (uint16)CV_FindFirstWhiteFromLeft(Output_Mask, y, 0, half);
        left_Black = (uint16)CV_FindFirstBlackFromLeft(Output_Mask, y, left, half);
        if(left_Black != left){
            flag++;
        }
    }
...
CV_Result_t CV_DetectMidline(uint16 *input_img, uint8* flag) {
    CV_Result_t result;
    static uint8 Output_Mask[CV_IMAGE_WIDTH * CV_IMAGE_HEIGHT];
    // 参数检查
    if (input_img == NULL) {
        result.valid = 0;
        result.error = 0;
    ...
    // 图像预处理：生成二值化掩码
    CV_PreprocessImage(input_img, Output_Mask);   //二值化后且完成腐蚀和膨胀
    
    *flag = CV_RI_Detect(Output_Mask);
    // 给大家两种计算中线偏差的方式，任选其一（方式二是课堂上讲过的计算画面两侧白色像素点差值的算法，感兴趣的同学可以研究）
    // 方法1：计算中线位置误差 （）
    
    result.error = CV_CalculateMidlineError(input_img, Output_Mask);
...
// 图像处理主函数
CV_Result_t CV_ProcessImage(uint8* flag) {
    CV_Result_t result;
    void *img_ptr;
    ...
    if (img_ptr != NULL) {
        // 进行中线检测
        result = CV_DetectMidline(img_ptr, flag);
