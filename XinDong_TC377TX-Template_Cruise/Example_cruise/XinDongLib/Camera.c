...
uint8 _atk_mc2640_set_gain(uint8 auto_gain, uint16 exposure_time, uint8 target_gain)
{
    if (auto_gain == 1) {
        _atk_mc2640_write_reg(0xff, 0x01);   // set to register bank 2
        uint8 com8_data = _atk_mc2640_read_reg(0x13);   // ACG reg
        com8_data |= (1 << 2);     // open auto gain control
        _atk_mc2640_write_reg(0x13, com8_data);
    } else {
        _atk_mc2640_write_reg(0xff, 0x01);   // set to register bank 2
        uint8 com8_data = _atk_mc2640_read_reg(0x13);   // ACG reg
        com8_data &= ~(1 << 2);     // close auto gain control
        com8_data &= ~(1 << 0);     // close auto exposure control
        _atk_mc2640_write_reg(0x13, com8_data);

        _atk_mc2640_write_reg(0x00, target_gain);   // gain reg

        uint8 reg45_data = _atk_mc2640_read_reg(0x45);
        reg45_data &= ~((1 << 6) - 1);
        reg45_data |= (uint8)(exposure_time >> 10);
        _atk_mc2640_write_reg(0x45, reg45_data);
        uint8 reg10_data = _atk_mc2640_read_reg(0x10);
        reg10_data = (uint8)(exposure_time >> 2);
        _atk_mc2640_write_reg(0x10, reg10_data);
        uint8 reg04_data = _atk_mc2640_read_reg(0x04);
        reg04_data &= ~0x03;
        reg04_data |= (uint8)(exposure_time & 0x03);
        _atk_mc2640_write_reg(0x10, reg04_data);
    }
    return ATK_MC2640_EOK;
}
...
//        }
    ...
    _atk_mc2640_set_gain(0, 1, 0);
    _atk_mc2640_set_output_speed(1, 80);                                     /* 输出速率 */
